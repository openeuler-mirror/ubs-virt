...
|环境项目|版本信息|
|:----|:----|
|操作系统|<ul><li>openEuler 22.03 LTS SP3</li><li>TencentOS Server 3.1</li></ul>|
|CPU架构|aarch64/x86|
|用户权限|安装与管理需 `root` 权限|
|软件要求|<ul><li>CMake ≥ 3.22.0</li><li>googletest ≥ 1.10.0</li><li>rapidjson  ≥ 1.2.0</li><li>clang ≥ 12.0.1</li><li>libbpf-devel ≥ 0.8.1</li><li>bpftool ≥ 6.8.0</li></ul>|
...
2. 拉取ubs-optimizer三方件。
    ...
    ![img.png](../images/image-20253106.png "拉取ubs-optimizer三方件")

3. 备份原build.sh，并修改build.sh为以下内容：
...
4. 进入到 ebpf/tests/目录，将其中的CMakeLists.txt，更新为:
...
1. 新增UT代码，并上传至环境中的virt-optimizer/ebpf/test目录下
