...
### 3 配置调优工具配置项
（1）配置文件路径为"/use/local/sbin/ubs-optimizer/config.json"，eBPF 指标采集配置说明如下，参考以下说明配置配置文件
...
ubs-opt stop_ebpf
```
（4）采集结束后，将虚拟机中采集的数据“/var/ubs-opt/data/data.json”拷贝至物理机“/var/ubs-opt/data/”路径下
**其中，性能数据示例如下：**

![img.png](../images/image-20253105.png)

（5）在host上启动性能优化器
```
ubs-opt-tuner start
```
UBS Optimizer会对虚拟机的性能数据进行分析，列出可执行的优化项，用户选择优化项，参考[性能优化方法](../optimize_operations/性能优化方法.md)，手动配置优化项
...
ubs-opt-tuner start
```
如下图，ubs-optimizer分析虚拟机的性能数据后，给出优化建议

![img.png](../images/image-20253102.png)

（4）根据当前应用场景在[性能优化方法](../optimize_operations/性能优化方法.md)中，找到对应的优化描述：
性能优化方法.md中，对应的优化项为GICv4.1以及HugePage 2M优化
...
![img.png](../images/image-20253103.png)

b.修改完成后，查看宿主机启动方式，执行对应命令，重新生成 GRUB2 的启动配置文件，并重启

![img.png](../images/image-20253104.png)
