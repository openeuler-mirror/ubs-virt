...
#define CHILD_TERM_GRACE_POLLS 100

LogConfig g_log_config = {
    .log_dir = "/var/log/enpu/enpu-manager/",
    .log_path = "eNPU_Manager.log",
...
static int mkdir_p(const char *path)
{
    ...
    char tmp[PATH_MAX];
    if (strncpy_s(tmp, sizeof(tmp), path, sizeof(tmp) - 1) != 0) {
        return ENPU_FAIL;
    }
    ...
    for (char *p = tmp + 1; *p != '\0'; p++) {
        ...
        *p = '\0';
        if (mkdir(tmp, LOG_DIR_MODE) != 0 && errno != EEXIST) {
            return ENPU_FAIL;
        }
        *p = '/';
    }
...
static bool wait_child_exit(pid_t pid)
{
    for (int i = 0; i < CHILD_TERM_GRACE_POLLS; i++) {
        pid_t ret = waitpid(pid, NULL, WNOHANG);
        if (ret == pid || (ret < 0 && errno != EINTR)) {
            return true;
        }
...
{
    /* 通知子进程停止，tar/find 对 SIGTERM 的缺省动作即终止 */
    if (kill(pid, SIGTERM) != 0 && errno != ESRCH) {
        fprintf(stderr, "[eNPU] kill_and_reap_child: SIGTERM %d failed.\n", (int)pid);
    }

    if (!wait_child_exit(pid)) {
        /* 子进程在限定时间内仍未退出，强行结束目标进程 */
        if (kill(pid, SIGKILL) != 0 && errno != ESRCH) {
            fprintf(stderr, "[eNPU] kill_and_reap_child: SIGKILL %d failed.\n", (int)pid);
        }
    }
    ...
    while (result == 0) {
        ...
        if (result == 0) {
            ...
            if (now.tv_sec > deadline.tv_sec || (now.tv_sec == deadline.tv_sec && now.tv_nsec >= deadline.tv_nsec)) {
                fprintf(stderr, "[eNPU] wait_process_timeout: timed out after %ds, killing pid %d.\n", timeout_sec,
                        (int)pid);
...
static pthread_rwlock_t g_log_gate = PTHREAD_RWLOCK_INITIALIZER;
...
int log_set_config(const char *log_dir, const char *log_level, size_t max_file_size, int max_backup_count,
                   int max_log_age)
{
    if (log_dir != NULL && log_dir[0] != '\0') {
        size_t dir_len = strlen(log_dir);
        size_t copy_len = (dir_len + 1 < FILE_PATH_LEN) ? dir_len : (FILE_PATH_LEN - 2);
        int ret = strncpy_s(g_log_config.log_dir, sizeof(g_log_config.log_dir), log_dir, copy_len);
        CHECK_COND_RETURN_ERROR_CODE_LOG(ret != 0, "Failed to set g_log_config.log_dir %s.", log_dir);
        if (copy_len > 0 && g_log_config.log_dir[copy_len - 1] != '/') {
            g_log_config.log_dir[copy_len] = '/';
            g_log_config.log_dir[copy_len + 1] = '\0';
        } else {
            g_log_config.log_dir[copy_len] = '\0';
        }
    }
    ...
    if (log_level != NULL && log_level[0] != '\0') {
        /* 兼容数字形式（如 "3"） */
        char *endptr = NULL;
        long num_val = strtol(log_level, &endptr, DECIMAL_BASE);
        if (endptr != log_level && *endptr == '\0' && num_val >= (long)ENPU_LOG_FATAL &&
            num_val <= (long)ENPU_LOG_DEBUG) {
            g_log_config.min_log_level = (EnpuLogLevel)num_val;
            ...
            g_log_config.min_log_level = ENPU_LOG_ERROR;
        } else if (strcmp(log_level, "fatal") == 0) {
            g_log_config.min_log_level = ENPU_LOG_FATAL;
        } else {
            fprintf(stderr, "[eNPU] Log config: invalid log_level='%s', using default (info).\n", log_level);
        }
    }
    ...
    if (max_backup_count > 0) {
        g_log_config.max_backup_count = max_backup_count;
    }

    if (max_log_age > 0) {
        g_log_config.max_log_age = max_log_age;
    }
...
int log_init(void)
{
    ...
    if (mkdir_p(g_log_config.log_dir) != ENPU_SUCCESS) {
        fprintf(stderr, "[eNPU] Log init failed: unable to create log dir %s.\n", g_log_config.log_dir);
    ...
    /* 环境变量优先级高于 log_set_config，仅当设置了才覆盖 */
    char *env_level = getenv("ENPU_LOG_LEVEL");
    if (env_level != NULL) {
        parse_level_env("ENPU_LOG_LEVEL", g_log_config.min_log_level, &g_log_config.min_log_level);
    }
...
void log_print(EnpuLogLevel level, const char *filename, int line, const char *format, ...)
{
    ...
    pthread_rwlock_rdlock(&g_log_gate);
    if (!g_log_initialized) {
        pthread_rwlock_unlock(&g_log_gate);
        fprintf(stderr, "[eNPU] Log module not initialized, cannot print log.\n");
        return;
    }
...
static int prepare_log_file(void)
{
    if (!g_log_config.log_file || (strcmp(g_log_config.log_path, g_log_config.log_file_path) != 0)) {
        ...
        int ret = strncpy_s(g_log_config.log_file_path, sizeof(g_log_config.log_file_path), g_log_config.log_path,
                            sizeof(g_log_config.log_file_path) - 1);
        if (ret != 0) {
            return ret;
        }
...
static int write_log_message(const LogMessage *msg, char *time_str, char *log_line)
{
    ...
    if (ret > 0) {
        ...
        if (g_log_config.console_output && msg->level <= ENPU_LOG_INFO) {
            /* 打屏用简洁格式，避免刷屏 */
            fprintf(stderr, "[%s] %s\n", log_level_str[msg->level], msg->message);
        }
    }
...
static void delete_expired_logs(void)
{
    if (g_log_config.max_log_age <= 0) {
        return;
    }

    char age_str[16];
    int ret = snprintf_s(age_str, sizeof(age_str), sizeof(age_str) - 1, "+%d", g_log_config.max_log_age);
    if (ret < 0) {
        return;
    }

    char *find_argv[] = {
        "find", (char *)g_log_config.log_dir, "-type", "f", "-name", "*.tar.gz", "-mtime", (char *)age_str, "-delete",
        NULL};
    (void)safe_exec(find_argv);
}
...
{
    pthread_rwlock_wrlock(&g_log_gate);
    if (!g_log_initialized || g_log_config.log_queue.shutdown) {
        pthread_rwlock_unlock(&g_log_gate);
        return;
    }
