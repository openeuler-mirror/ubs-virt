...
bool memory_check(size_t requested)
{
    ...
    if(requested + used > quota){
        LOG_ERROR("out of memeory, request %zd B, used %zd B, quota %zd B", requested, used, quota);
        return false;
    }
    return true;
}
...
int guard_memory(size_t requested){
    ...
    if (!file_lock_isvalid(&lock)){
        LOG_ERROR("Guard memeory failed to create file lock!");
        file_lock_destroy(&lock);
        return ACL_ERROR_FAILURE;
    }

    if (!memory_check(requested)){
        LOG_ERROR("Guard memory out of memeory error for requested:%zd", requested);
        file_lock_destroy(&lock);
